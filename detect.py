...
if __name__ == '__main__':
    ...
    while True:
        ...
        nh, nw = image_np.shape[:2]
        max_height = 900
        max_width = 900
        # only shrink if img is bigger than required
        if max_height < nh or max_width < nw:
            # get scaling factor
            scaling_factor = max_height / float(nh)
            if max_width/float(nw) < scaling_factor:
                scaling_factor = max_width / float(nw)
            # resize image
        ...
        if (args.display > 0):
            ...
            cv2.imshow('hands detection', cv2.cvtColor(
        ...
        wait = input("ENTER for next frame")
